...
function TMModule({onBack}){
  ...
  const [sheetDraft,setSheetDraft]=useState({date:"",sheet_number:"",description:"",labour:"",equipment:"",material:"",subcontractor:"",markup:"",total:"",signed:false});
...
{"document_type":"Signed field sheet|Priced daily breakdown|Written direction|Summary of extra work|RCO|Change order|Progress invoice|Receipt / backup|Mixed package|Other","project":"","contractor":"","scope_title":"","location":"","description":"","direction_ref":"","rco_number":"","change_order_number":"","invoice_number":"","invoice_date":"YYYY-MM-DD or empty","invoice_amount_before_tax":number or null,"timesheets":[{"date":"YYYY-MM-DD or empty","field_sheet_number":"","priced_sheet_number":"","description":"","labour":number,"equipment":number,"material":number,"subcontractor":number,"markup":number,"total":number,"signed_by_southwest":true|false}],"notes":"brief extraction notes"}`;
  ...
  function confirmNewTMForm(){
    ...
    setItems(prev=>[{...intakeReview,id,created_at:new Date().toISOString()},...prev]);setSelected(null);setIntakeOpen(false);setIntakeFile(null);setIntakeReview(null);setIntakeNotice("");showToast(`${id} added from uploaded form ✓`);
  }
  async function uploadTMDocument(){
    ...
    const existingKeys=new Set((selected.timesheets||[]).map(t=>`${t.date}|${t.sheet_number}|${t.priced_sheet_number}`));
    const extractedSheets=(extracted?.timesheets||[]).map((t,i)=>({id:Date.now()+i,date:t.date||"",sheet_number:t.field_sheet_number||"",priced_sheet_number:t.priced_sheet_number||"",description:t.description||"",labour:+t.labour||0,equipment:+t.equipment||0,material:+t.material||0,subcontractor:+t.subcontractor||0,markup:+t.markup||0,total:+t.total||0,signed:!!t.signed_by_southwest})).filter(t=>!existingKeys.has(`${t.date}|${t.sheet_number}|${t.priced_sheet_number}`));
    ...
    const updated={...selected,
      project:selected.project||extracted?.project||"",trade:selected.trade||extracted?.contractor||"",title:selected.title||extracted?.scope_title||"",location:selected.location||extracted?.location||"",direction_ref:selected.direction_ref||extracted?.direction_ref||"",rco_number:selected.rco_number||extracted?.rco_number||"",change_order_number:selected.change_order_number||extracted?.change_order_number||"",
      timesheets:[...(selected.timesheets||[]),...extractedSheets],invoices:[...(selected.invoices||[]),...extractedInvoices],documents:[...(selected.documents||[]),{id:Date.now(),type,reference:docDraft.reference||extracted?.invoice_number||extracted?.rco_number||extracted?.change_order_number||"",filename:docDraft.file.name,file_url:url,added_at:new Date().toISOString(),extraction_notes:extracted?.notes||""}]
    };
    saveItem(updated);setDocDraft({type:"Auto-detect",reference:"",file:null});setLoading(false);showToast(extracted?`Document read and linked${extractedSheets.length?` · ${extractedSheets.length} timesheet${extractedSheets.length>1?"s":""} added`:""} ✓`:"Document stored — AI extraction needs manual review","err");
  }
...
function TMDetail({item,onClose,onSave,onDelete,sheetDraft,setSheetDraft,onAddSheet,invoiceDraft,setInvoiceDraft,onAddInvoice,docDraft,setDocDraft,onUpload,loading}){
  ...
  return <Modal title={`${item.id} · ${item.title}`} onClose={onClose} wide><div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:18}}>{[["summary","Summary"],["timesheets","Priced T&M"],["documents","Documents"],["invoices","Progress Billing"]].map(([v,label])=><button key={v} onClick={()=>setView(v)} style={{...ghostBtn,color:view===v?C.yellow:C.sub,borderColor:view===v?C.yellow:C.border}}>{label}</button>)}</div>
    ...
    {view==="timesheets"&&<><table style={tableStyle}><thead><tr>{["Date","Field sheet #","Priced sheet #","Labour","Equipment","Material","Markup","Total","Signed",""].map(h=><th key={h} style={thStyle}>{h}</th>)}</tr></thead><tbody>{(item.timesheets||[]).map(t=><tr key={t.id}><td style={tdStyle}>{t.date}</td><td style={{...tdStyle,color:t.priced_sheet_number&&t.priced_sheet_number!==t.sheet_number?C.red:C.text}}>{t.sheet_number||"—"}</td><td style={{...tdStyle,color:t.priced_sheet_number&&t.priced_sheet_number!==t.sheet_number?C.red:C.text}}>{t.priced_sheet_number||"—"}</td><td style={tdNum}>{money(t.labour)}</td><td style={tdNum}>{money(t.equipment)}</td><td style={tdNum}>{money(t.material)}</td><td style={tdNum}>{money(t.markup)}</td><td style={{...tdNum,fontWeight:800}}>{money(t.total)}</td><td style={tdStyle}>{t.signed?"✓":"⚠"}</td><td style={tdStyle}><span style={{display:"flex",gap:4}}><button onClick={()=>setEditingSheet({...t})} style={miniEdit}>Edit</button><button onClick={()=>{if(window.confirm(`Delete timesheet ${t.sheet_number||t.date}?`))onSave({...item,timesheets:item.timesheets.filter(x=>x.id!==t.id)})}} style={miniDanger}>Delete</button></span></td></tr>)}</tbody></table><section style={{...panel,marginTop:18}}><div style={panelTitle}>Add timesheet / priced breakdown</div><div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(120px,1fr))",gap:10}}>{sf("Work date","date","date")}{sf("Field sheet #","sheet_number","text")}{sf("Priced sheet #","priced_sheet_number","text")}{sf("Labour","labour")}{sf("Equipment","equipment")}{sf("Materials","material")}{sf("Subcontractors","subcontractor")}{sf("Markup","markup")}{sf("Entered total (optional)","total")}</div>{sf("Work description","description","text")}<label style={{display:"flex",gap:8,color:C.sub,fontSize:12,marginBottom:14}}><input type="checkbox" checked={sheetDraft.signed} onChange={e=>setSheetDraft({...sheetDraft,signed:e.target.checked})}/>Signed field sheet is present</label><button onClick={onAddSheet} style={primaryBtn}>Add Timesheet</button></section></>}
    ...
    {editingSheet&&<RecordEditor title="Edit Timesheet" draft={editingSheet} setDraft={setEditingSheet} fields={[["Work date","date","date"],["Field sheet #","sheet_number"],["Priced sheet #","priced_sheet_number"],["Description","description"],["Labour","labour","number"],["Equipment","equipment","number"],["Materials","material","number"],["Subcontractors","subcontractor","number"],["Markup","markup","number"],["Total","total","number"]]} checkbox={["Signed field sheet is present","signed"]} onClose={()=>setEditingSheet(null)} onSave={()=>{onSave({...item,timesheets:item.timesheets.map(x=>x.id===editingSheet.id?editingSheet:x)});setEditingSheet(null)}}/>}
...
function TMIntakeReview({draft,setDraft,notice,onBack,onConfirm}){
  ...
  const addBlankSheet=()=>setDraft({...draft,timesheets:[...draft.timesheets,{id:Date.now(),date:"",sheet_number:"",priced_sheet_number:"",description:"",labour:0,equipment:0,material:0,subcontractor:0,markup:0,total:0,signed:false}]});
  return <>{notice&&<div style={{background:"#451a03",border:`1px solid ${C.yellow}`,color:"#fde68a",borderRadius:9,padding:"10px 13px",marginBottom:15,fontSize:12}}>⚠ {notice}</div>}<div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(150px,1fr))",gap:10}}>{f("Project","project")}{f("Trade / contractor","trade")}{f("T&M scope / title","title")}{f("Location","location")}{f("Written direction","direction_ref")}{f("RCO number","rco_number")}{f("Change order number","change_order_number")}<label style={formLabel}>Stage<select value={draft.stage} onChange={e=>setDraft({...draft,stage:e.target.value})} style={inputStyle}>{TM_STAGES.map(s=><option key={s}>{s}</option>)}</select></label></div><label style={formLabel}>Work description<textarea value={draft.description||""} onChange={e=>setDraft({...draft,description:e.target.value})} style={{...inputStyle,minHeight:65,resize:"vertical"}}/></label><section style={{...panel,marginTop:12,padding:14}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,marginBottom:10}}><div><div style={panelTitle}>Timesheets read from form</div><div style={{color:C.muted,fontSize:11}}>Check the sheet numbers and amounts against the original form.</div></div><button onClick={addBlankSheet} style={ghostBtn}>+ Add Missing Row</button></div>{draft.timesheets.length?<div style={{overflowX:"auto"}}><table style={tableStyle}><thead><tr>{["Date","Field sheet #","Priced sheet #","Description","Labour","Equipment","Materials","Subtrades","Markup","Total","Signed",""].map(h=><th key={h} style={thStyle}>{h}</th>)}</tr></thead><tbody>{draft.timesheets.map(t=><tr key={t.id}><td style={tdStyle}><input type="date" value={t.date||""} onChange={e=>updateSheet(t.id,"date",e.target.value)} style={{...inputStyle,minWidth:130}}/></td><td style={tdStyle}><input value={t.sheet_number||""} onChange={e=>updateSheet(t.id,"sheet_number",e.target.value)} style={{...inputStyle,minWidth:105}}/></td><td style={tdStyle}><input value={t.priced_sheet_number||""} onChange={e=>updateSheet(t.id,"priced_sheet_number",e.target.value)} style={{...inputStyle,minWidth:105}}/></td><td style={tdStyle}><input value={t.description||""} onChange={e=>updateSheet(t.id,"description",e.target.value)} style={{...inputStyle,minWidth:190}}/></td>{["labour","equipment","material","subcontractor","markup","total"].map(k=><td key={k} style={tdStyle}><input type="number" value={t[k]??""} onChange={e=>updateSheet(t.id,k,e.target.value)} style={{...inputStyle,minWidth:90}}/></td>)}<td style={tdStyle}><input type="checkbox" checked={!!t.signed} onChange={e=>updateSheet(t.id,"signed",e.target.checked)}/></td><td style={tdStyle}><button onClick={()=>removeSheet(t.id)} style={miniDanger}>Remove</button></td></tr>)}</tbody></table></div>:<div style={{color:C.muted,padding:"20px 0",textAlign:"center"}}>No daily rows were found. Add a missing row or confirm the header details and add timesheets later.</div>}</section>{draft.invoices?.length>0&&<section style={{...panel,marginTop:12,padding:14}}><div style={panelTitle}>Invoice found</div>{draft.invoices.map(v=><div key={v.id} style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 2fr",gap:10}}><label style={formLabel}>Invoice #<input value={v.invoice_number||""} onChange={e=>setDraft({...draft,invoices:draft.invoices.map(x=>x.id===v.id?{...x,invoice_number:e.target.value}:x)})} style={inputStyle}/></label><label style={formLabel}>Date<input type="date" value={v.date||""} onChange={e=>setDraft({...draft,invoices:draft.invoices.map(x=>x.id===v.id?{...x,date:e.target.value}:x)})} style={inputStyle}/></label><label style={formLabel}>Amount before HST<input type="number" value={v.amount??""} onChange={e=>setDraft({...draft,invoices:draft.invoices.map(x=>x.id===v.id?{...x,amount:e.target.value}:x)})} style={inputStyle}/></label><label style={formLabel}>Timesheets covered<input value={v.timesheet_refs||""} onChange={e=>setDraft({...draft,invoices:draft.invoices.map(x=>x.id===v.id?{...x,timesheet_refs:e.target.value}:x)})} style={inputStyle}/></label></div>)}</section>}<div style={{background:C.blue+"12",border:`1px solid ${C.blue}44`,borderRadius:9,padding:"10px 13px",marginTop:14,color:C.sub,fontSize:12}}>Source file: <b style={{color:C.text}}>{draft.documents?.[0]?.filename||"Uploaded form"}</b>. The original stays attached to this T&M item for later checking.</div><div style={modalActions}><button onClick={onConfirm} style={primaryBtn}>Confirm & Add to Register</button><button onClick={onBack} style={ghostBtn}>Choose Different Form</button></div></>
}
